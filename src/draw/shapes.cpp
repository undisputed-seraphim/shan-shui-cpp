...
void stroke(Painter& p, const Pts& ptlist, const SArg& a) {
	...
	for (const auto& v : vtxlist)
		shifted.push_back({v[0] + a.xof, v[1] + a.yof});
...
void texture(Painter& p, const std::vector<Pts>& ptlist, const TArg2& a) {
	...
	for (int i = 0; i < a.tex; i++) {
		...
		for (int j = start; j < end; j++) {
			...
			double x = ptlist[(size_t)std::floor(layer)][j][0] * pp + ptlist[(size_t)std::ceil(layer)][j][0] * (1 - pp);
			double y = ptlist[(size_t)std::floor(layer)][j][1] * pp + ptlist[(size_t)std::ceil(layer)][j][1] * (1 - pp);
