...
void Painter::absorb(Painter&& o) {
  ops_.insert(ops_.end(),
              std::make_move_iterator(o.ops_.begin()),
              std::make_move_iterator(o.ops_.end()));
  o.ops_.clear();
}
...
std::string Painter::toSvg() const {
  std::string s;
  size_t est = 0;
  for (const auto& op : ops_) est += op.pts.size() * 16 + 64;
  s.reserve(est);
  for (const auto& op : ops_) {
    if (op.kind == 0) {
      ...
      std::snprintf(buf, sizeof buf,
                    ...
                    "rotate(%s)'>%s</text>",
                    fmtNum(op.fontSize).c_str(), op.fontFamily.c_str(),
                    op.fill.c_str(), fmtNum(op.x).c_str(), fmtNum(op.y).c_str(),
                    fmtNum(op.rot).c_str(), op.content.c_str());
      s += buf;
    }
  }
  return s;
