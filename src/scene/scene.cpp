...
std::vector<Plan> mountplanner(Scene& sc, double xmin, double xmax) {
  auto locmax = [](double x, double y,
                   const std::function<double(double, double)>& f, int r) {
    double z0 = f(x, y);
  ...
  auto ns = [&](double x, double y) {
    return std::max(nse(x * samp) - 0.55, 0.0) * 2;
  };
  auto nns = [&](double x) { return 1 - nse(x * samp); };
  auto nnns = [&](double x, double y) {
    return std::max(nse(x * samp * 2, 2) - 0.55, 0.0) * 2;
  };
  ...
  for (double i = xmin; i < xmax; i += xstep) {
    for (double j = 0; j < yr(i) * 480; j += 30) {
      if (locmax(i, j, ns, 2)) {
        ...
        if (chadd(r)) {
          for (int k = (int)std::floor((xof - mwid) / xstep);
               k < (xof + mwid) / xstep; k++) {
            // JS: planmtx[k] += 1 (undefined + 1 = NaN)
            auto it = sc.planmtx.find(k);
            if (it == sc.planmtx.end()) sc.planmtx[k] = std::nan("");
            else it->second += 1;
          }
        }
      }
...
std::string Scene::calcViewBox() const {
  double zoom = 1.142;
  return fmtNum(cursx) + " 0 " + fmtNum(windx / zoom) + " " +
         fmtNum(windy / zoom);
}
...
std::string genMountain(double x, double y, double seed, double hei, double wid,
                        int tex) {
  Painter sp;
