...
void Noise::ensure() {
  if (!perlin.empty()) return;
  perlin.assign(SIZE + 1, 0.0);
  for (int i = 0; i < SIZE + 1; i++) perlin[i] = rnd();
}

static double scaledCosine(double i) {
  return 0.5 * (1.0 - std::cos(i * M_PI));
}
...
double Noise::noise(double x, double y, double z) {
  ...
  for (int o = 0; o < perlinOctaves; o++) {
    ...
    int32_t of = (int32_t)(xi + (double)((int32_t)yi << YWRAPB) +
                           (double)((int32_t)zi << ZWRAPB));
    rxf = scaledCosine(xf);
    ryf = scaledCosine(yf);
...
void Noise::noiseDetail(double lod, double falloff) {
  if (lod > 0) perlinOctaves = (int)lod;
