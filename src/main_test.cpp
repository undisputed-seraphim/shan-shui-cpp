...
int main() {
  // --- PRNG stream for seed "123" (from ref_prng.txt) ---
  {
    Rng::inst().seed("123");
    const char* expect[] = {
        "0.563566544187486", "0.321631826709592", "0.265661911199784",
        "0.977427078879056", "0.717024806592673", "0.017856904721004",
        "0.305112297321373", "0.429255656289695", "0.576170941272075",
        "0.628078658324792", "0.461861817647186", "0.649525784552932"};
    bool ok = true;
    std::string got;
    for (int i = 0; i < 12; i++) {
      std::string v = toFixed(rnd(), 15);
      if (v != expect[i]) { ok = false; got += v + " "; }
    }
  ...
  // --- noise head for seed "123" (from ref_noise.txt) ---
  {
    ...
    const char* expect[] = {"0.528343635", "0.379729339", "0.352870176",
                            "0.455091527", "0.423446516", "0.424147296",
                            "0.673503669", "0.591876120"};
    bool ok = true;
    std::string got;
    for (int i = 0; i < 8; i++) {
  ...
  // --- full view for seed "123" ---
  {
    ...
    f << svg;
    std::cout << "view: xmin " << sc.xmin << " xmax " << sc.xmax
              << " chunks " << sc.chunks.size() << " svg bytes " << svg.size()
              << "\n";
