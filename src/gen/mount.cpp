...
void foot(Painter& p, const std::vector<Pts>& ptlist, double xof, double yof) {
  ...
  for (size_t i = 0; i + 2 < ptlist.size(); i++) {
    if ((int)i == ni) {
      ni = std::min(ni + (int)randChoice<double>({1.0, 2.0}),
                    (int)ptlist.size() - 1);
      ftlist.push_back({});
      ftlist.push_back({});
      ...
      for (double j = 0; j < jmax; j++) {
        ftlist[ftlist.size() - 2].push_back(
            {ptlist[i][j][0] + nse(j * 0.1, (double)i) * 10, ptlist[i][j][1]});
        ftlist[ftlist.size() - 1].push_back(
            {ptlist[i][ptlist[i].size() - 1 - j][0] - nse(j * 0.1, (double)i) * 10,
             ptlist[i][ptlist[i].size() - 1 - j][1]});
      }
      std::reverse(ftlist[ftlist.size() - 2].begin(),
                   ftlist[ftlist.size() - 2].end());
      std::reverse(ftlist[ftlist.size() - 1].begin(),
                   ftlist[ftlist.size() - 1].end());
      for (int j = 0; j < (int)span; j++) {
        double pp = j / span;
        double x1 = ptlist[i][0][0] * (1 - pp) + ptlist[ni][0][0] * pp;
        double y1 = ptlist[i][0][1] * (1 - pp) + ptlist[ni][0][1] * pp;
        double x2 = ptlist[i][ptlist[i].size() - 1][0] * (1 - pp) +
                    ptlist[ni][ptlist[i].size() - 1][0] * pp;
        double y2 = ptlist[i][ptlist[i].size() - 1][1] * (1 - pp) +
                    ptlist[ni][ptlist[i].size() - 1][1] * pp;
  ...
  for (const auto& row : ftlist) {
    ...
    sa.wid = 1;
    Pts shifted;
    for (const auto& v : row) shifted.push_back({v[0] + xof, v[1] + yof});
    stroke(p, shifted, sa);
  }
}
...
void Mount::mountain(Painter& p, double xoff, double yoff, double seed,
                     const MountainArg& a) {
  ...
  vegetate(
      [&](double x, double y) {
        std::string col =
            "rgba(100,100,100," +
            toFixed(nse(0.01 * x, 0.01 * y) * 0.5 * 0.3 + 0.5, 3) + ")";
        Tree::tree02(p, x + xoff, y + yoff - 5, 16, 8, 2, col, 0.5);
      },
      [&](int i, int j) {
        double ns = nse(j * 0.1, seed);
        return i == 0 && ns * ns * ns < 0.1 &&
               std::fabs(ptlist[i][j][1]) / h > 0.2;
      },
  ...
  // OUTLINE
  {
    ...
    sa.wid = 3;
    Pts shifted;
    for (const auto& v : ptlist[0]) shifted.push_back({v[0] + xoff, v[1] + yoff});
    stroke(p, shifted, sa);
  }
  ...
  if (veg) {
    ...
    vegetate(
        [&](double x, double y) {
          ...
          std::string col =
              "rgba(100,100,100," +
              toFixed(nse(0.01 * x, 0.01 * y) * 0.5 * 0.3 + 0.3, 3) + ")";
          Tree::tree01(p, x + xoff, y + yoff, ht, rnd() * 3 + 1, col, 0.5);
        },
        [&](int i, int j) {
          double ns = nse(i * 0.2, j * 0.05, seed);
          return j % 2 && ns * ns * ns * ns < 0.012 &&
                 std::fabs(ptlist[i][j][1]) / h < 0.3;
        },
        [](const Pts& veglist, int i) {
          int counter = 0;
          for (size_t j = 0; j < veglist.size(); j++) {
            if ((int)j != i &&
                std::pow(veglist[i][0] - veglist[j][0], 2) +
                        std::pow(veglist[i][1] - veglist[j][1], 2) <
                    30 * 30) {
              counter++;
            }
    ...
    vegetate(
        [&](double x, double y) {
          ...
          std::string col =
              "rgba(100,100,100," +
              toFixed(nse(0.01 * x, 0.01 * y) * 0.5 * 0.3 + 0.3, 3) + ")";
          Tree::tree03(p, x + xoff, y + yoff, ht, 5,
                       [bc, bp](double xx) { return std::pow(xx * bc, bp); }, col,
                       0.5);
        },
        [&](int i, int j) {
          double ns = nse(i * 0.2, j * 0.05, seed);
  ...
  vegetate(
      [&](double x, double y) {
        ...
        if (tt == 1) {
          ...
          Arch::arch02(p, x + xoff, y + yoff, seed, 10, w, rot, 5, sto, sty,
                       false);
        } else if (tt == 2) {
          Arch::arch04(p, x + xoff, y + yoff, seed, 15, 30, 0.7,
                       (double)randChoice<double>({1, 1, 1, 2, 2}), 1);
        }
      },
      [&](int i, int j) {
  ...
  vegetate(
      ...
      [&](int i, int j) {
        return i == 1 && std::fabs((double)j - ptlist[i].size() / 2) < 1 &&
               rnd() < 0.02;
      },
      [](const Pts&, int) { return true; });
  ...
  vegetate(
      [&](double x, double y) {
        Arch::transmissionTower01(p, x + xoff, y + yoff, seed);
      },
      [&](int i, int j) {
  ...
  vegetate(
      ...
      [&](int i, int j) {
        return (j == 0 || j == (int)ptlist[i].size() - 1) && rnd() < 0.1;
      },
      [](const Pts&, int) { return true; });
...
void flatDec(Painter& p, double xoff, double yoff, double xmin, double xmax,
  ...
  if (tt == 1) {
    ...
    for (double i = x0; i < x1; i += 30) {
      {
        ...
        Tree::tree05(p, tx, yoff + (ymin + ymax) / 2 + 20, th, 5,
                     "rgba(100,100,100,0.5)", 0.5);
      }
    }
    ...
    for (int i = 0; i < (int)randChoice<double>({1, 1, 1, 1, 2, 2, 3}); i++) {
      ...
      Tree::tree04(p, xoff + xr, yoff + yr + 20, 300, 6, "rgba(100,100,100,0.5)",
                   0.5);
    ...
    for (int i = 0; i < (int)randChoice<double>({1, 1, 1, 1, 2, 2, 3}); i++) {
      {
        ...
        double th = 60 + rnd() * 60;
        Tree::tree06(p, tx, yoff + (ymin + ymax) / 2, th, 6,
                     "rgba(100,100,100,0.5)", 0.5);
      }
    }
  } else if (tt == 4) {
...
void Mount::flatMount(Painter& p, double xoff, double yoff, double seed,
                      const FlatMountArg& a) {
  ...
  p.poly(grlist, PArg{.xof = xoff, .yof = yoff, .fil = "white", .str = "none",
                      .wid = 2});
  {
    SArg sa;
    sa.wid = 3;
    ...
    Pts shifted;
    for (const auto& v : grlist) shifted.push_back({v[0] + xoff, v[1] + yoff});
    stroke(p, shifted, sa);
  }
...
void Mount::distMount(Painter& p, double xoff, double yoff, double seed,
                      const DistMountArg& a) {
  double hei = a.hei;
  double len = a.len;
  int seg = a.seg;
  ...
  for (int i = 0; i < (int)(len / span / seg); i++) {
    ...
    for (int j = 0; j < seg + 1; j++) {
      ...
      row.push_back(
          {xoff + k * span,
           yoff - hei * nse(k * 0.05, seed) *
                      std::pow(std::sin((PI * k) / (len / span)), 0.5)});
    }
    for (int j = 0; j < seg / 2.0 + 1; j++) {
      double k = i * seg + j * 2;
      row.insert(row.begin(),
                 {xoff + k * span,
                  yoff + 24 * nse(k * 0.05, 2, seed) *
                             std::pow(std::sin((PI * k) / (len / span)), 1)});
    }
    ptlist.push_back(std::move(row));
  }
  for (const auto& row : ptlist) {
    auto getCol = [&](double x, double y) {
      ...
      return "rgb(" + std::to_string(c) + "," + std::to_string(c) + "," +
             std::to_string(c) + ")";
    };
    std::string col = getCol(row[row.size() - 1][0], row[row.size() - 1][1]);
    p.poly(row, PArg{.fil = col, .str = "none", .wid = 1});

    auto T = PolyTools::triangulate(row,
                                    TriArgs{.area = 100, .convex = true,
                                            .optimize = false});
...
void Mount::rock(Painter& p, double xoff, double yoff, double seed,
                 double hei, double wid, int tex, int ret, double sha) {
  const int reso0 = 10, reso1 = 50;
  std::vector<Pts> ptlist;
  for (int i = 0; i < reso0; i++) {
    ...
    for (int j = 0; j < reso1; j++) {
      ...
      double l = (wid * hei) /
                 std::sqrt(std::pow(hei * std::cos(a), 2) +
                           std::pow(wid * std::sin(a), 2));
      l *= 0.7 + 0.3 * nslist[j];
...

  {
    ...
    ta.sha = sha;
    ta.col = [](double) {
      return "rgba(180,180,180," + toFixed(0.3 + rnd() * 0.3, 3) + ")";
    };
    ta.dis = []() {
