...
std::pair<Pts, Pts> branch(const BranchArg& a) {
  ...
  for (int i = 0; i < tl; i++) {
    ...
    trlist1.push_back({nxx + std::cos(ang2 + PI / 2) * (nw + woff + b),
...
void twig(Painter& p, double tx, double ty, int dep, const TwigArg& a) {
  ...
  auto tfun = [](double x, int i) {
    return -1 / std::pow((double)i / tl + 1, 5) + 1;
  };
  ...
  for (int i = 0; i < tl; i++) {
    ...
    if ((i == ((tl / 3) | 0) || i == (((tl * 2) / 3) | 0)) && dep > 0) {
      twig(p, nx + tx, ny + ty, dep - 1,
           TwigArg{.dir = a.dir * randChoice<double>({-1.0, 1.0}),
                   .sca = a.sca * 0.8,
                   .wid = a.wid,
                   .ang = a.ang,
                   .hasLea = a.hasLea,
    ...
    if (i == tl - 1 && a.hasLea) {
      for (int j = 0; j < 5; j++) {
        ...
        blob(p, nx + tx + std::cos(a.ang) * dj * a.wid,
             ny + ty + (std::sin(a.ang) * dj - a.lea / (dep + 1)) * a.wid, ba);
      }
    }
  }
  SArg sa;
...
void bark(Painter& p, double x, double y, double wid, double ang) {
  ...
  auto fun = [](double x) {
    return x <= 1 ? std::pow(std::sin(x * PI), 0.5)
                  : -std::pow(std::sin((x + 1) * PI), 0.5);
  };
  ...
  double n0 = rnd() * 10;
  for (int i = 0; i < (int)reso + 1; i++) nslist.push_back(nse(i * 0.05, n0));
  loopNoise(nslist);
  Pts brklist;
...
void barkify(Painter& p, double x, double y, std::pair<Pts, Pts>& tr) {
  ...
  for (size_t i = 2; i + 1 < tr0.size(); i++) {
    ...
    if (rnd() < 0.05) {
      ...
      for (double j = 0; j < jl; j++) {
        ...
        ba.ang = a0 + PI / 2;
        ba.col = "rgba(100,100,100,0.6)";
        blob(p, xya[0] + x + std::cos(xya[2]) * (j - jl / 2) * 4,
             xya[1] + y + std::sin(xya[2]) * (j - jl / 2) * 4, ba);
      }
    }
  }
  ...
  for (size_t i = 0; i < rglist.size(); i++) {
    ...
    for (size_t j = 0; j < divRow.size(); j++) {
      divRow[j][0] +=
          (nse((double)i, j * 0.1, 1) - 0.5) * (15 + 5 * randGaussian());
      divRow[j][1] +=
          (nse((double)i, j * 0.1, 2) - 0.5) * (15 + 5 * randGaussian());
    }
    ...
    sa.out = 0;
    Pts shifted;
    for (const auto& v : divRow) shifted.push_back({v[0] + x, v[1] + y});
    stroke(p, shifted, sa);
  }
}
...
void Tree::tree01(Painter& p, double x, double y, double hei, double wid,
                  const std::string& col, double noi) {
  const int reso = 10;
...
void Tree::tree02(Painter& p, double x, double y, double hei, double wid,
                  int clu, const std::string& col, double noi) {
  for (int i = 0; i < clu; i++) {
...
void Tree::tree03(Painter& p, double x, double y, double hei, double wid,
                  const std::function<double(double)>& ben, const std::string& col,
                  double noi) {
  ...
  for (int i = 0; i < reso; i++) {
    ...
    line1.push_back(
        {nx + ((nslist[i][0] - 0.5) * wid - wid / 2) * (reso - i) / reso, ny});
    line2.push_back(
        {nx + ((nslist[i][1] - 0.5) * wid + wid / 2) * (reso - i) / reso, ny});
  }
  Pts lc = line1;
...
void Tree::tree04(Painter& p, double x, double y, double hei, double wid,
                  const std::string& col, double noi) {
  Painter tx, tw;
  ...
  for (size_t i = 0; i < trlist.size(); i++) {
    if ((i >= trlist.size() * 0.3 && i <= trlist.size() * 0.7 && rnd() < 0.1) ||
        i == trlist.size() / 2 - 1) {
      double ba = PI * 0.2 - PI * 1.4 * (i > trlist.size() / 2);
      auto br = branch(BranchArg{.hei = hei * (rnd() + 1) * 0.3,
                                 .wid = wid * 0.5,
                                 .ang = ba});
      ...
      auto brCopy = br;
      for (auto& v : brCopy.first) { v[0] += trlist[i][0]; v[1] += trlist[i][1]; }
      for (auto& v : brCopy.second) { v[0] += trlist[i][0]; v[1] += trlist[i][1]; }
      ...
      for (size_t j = 0; j < br.first.size(); j++) {
        if (rnd() < 0.2 || j == br.first.size() - 1) {
          ...
          ta.sca = (0.5 * hei) / 300;
          ta.dir = ba > -PI / 2 ? 1 : -1;
          twig(tw, br.first[j][0] + trlist[i][0] + x,
               br.first[j][1] + trlist[i][1] + y, 1, ta);
        }
      }
      Pts brall = br.first;
  ...
  sa.out = 0;
  Pts shifted;
  for (const auto& v : trmlist) shifted.push_back({v[0] + x, v[1] + y});
  stroke(p, shifted, sa);
...
void Tree::tree05(Painter& p, double x, double y, double hei, double wid,
                  const std::string& col, double noi) {
  Painter tx, tw;
  ...
  for (size_t i = 0; i < trlist.size(); i++) {
    ...
    if ((i >= trlist.size() * 0.2 && i <= trlist.size() * 0.8 && i % 3 == 0 &&
         rnd() > pp) ||
        i == trlist.size() / 2 - 1) {
      double bar = rnd() * 0.2;
      double ba =
          -bar * PI - (1 - bar * 2) * PI * (i > trlist.size() / 2);
      auto br = branch(BranchArg{.hei = hei * (0.3 * pp - rnd() * 0.05),
                                 .wid = wid * 0.5,
                                 .ang = ba,
                                 .ben = 0.5});
      br.first.erase(br.first.begin());
      ...
      for (size_t j = 0; j < br.first.size(); j++) {
        if (j % 20 == 0 || j == br.first.size() - 1) {
          ...
          ta.lea = 5;
          twig(tw, br.first[j][0] + trlist[i][0] + x,
               br.first[j][1] + trlist[i][1] + y, 0, ta);
        }
      }
      Pts brall = br.first;
      brall.insert(brall.end(), br.second.rbegin(), br.second.rend());
...
// tree06's fracTree
Pts fracTree6(Painter& tx, Painter& tw, double xoff, double yoff, int dep,
              double hei, double wid, double ang, double ben) {
  auto tr = branch(BranchArg{.hei = hei, .wid = wid, .ang = ang, .det = hei / 20,
                             .ben = ben});
  ...
  for (size_t i = 0; i < trlist.size(); i++) {
    ...
    if (((rnd() < 0.025 && i >= trlist.size() * 0.2 && i <= trlist.size() * 0.8) ||
      ...
      Pts brlist = fracTree6(tx, tw, trlist[i][0] + xoff, trlist[i][1] + yoff,
                             dep - 1, hei * (0.7 + rnd() * 0.2), wid * 0.6,
                             ang + ba, 0.55);
      ...
      for (size_t j = 0; j < brlist.size(); j++) {
        if (rnd() < 0.03) {
          ...
          ta.hasLea = false;
          ta.lea = 0;
          twig(tw, brlist[j][0] + trlist[i][0] + xoff,
               brlist[j][1] + trlist[i][1] + yoff, 2, ta);
        }
      }
...
// tree08's fracTree
void fracTree8(Painter& p, double xoff, double yoff, int dep, double ang,
               double len, double ben) {
  auto fun = dep == 0 ? [](double x) { return std::cos(0.5 * PI * x); }
                      : [](double) { return 1.0; };
  Pt spt = {xoff, yoff};
  ...
  if (dep != 0) {
    ...
    if (rnd() < 0.5) {
      ...
      double angA = ang + ben +
                    PI * randChoice<double>({normRand(-1, 0.5), normRand(0.5, 1)}) * 0.2;
...
void Tree::tree06(Painter& p, double x, double y, double hei, double wid,
                  const std::string& col, double noi) {
  Painter tx, tw;
...
void Tree::tree07(Painter& p, double x, double y, double hei, double wid,
                  const std::function<double(double)>& ben, const std::string& col,
                  double noi) {
  ...
  for (int i = 0; i < reso; i++) {
    ...
    if (i >= reso / 4.0) {
      for (int j = 0; j < 1; j++) {
        ...
        auto tris = PolyTools::triangulate(bpl, TriArgs{.area = 50, .convex = true,
                                                        .optimize = false});
  ...
  Pts lc = line1;
  lc.insert(lc.end(), line2.rbegin(), line2.rend());
  auto tris = PolyTools::triangulate(lc, TriArgs{.area = 50, .convex = true,
                                                 .optimize = true});
  ...
  for (const auto& tri : tris) {
    ...
    std::string co = "rgba(" + std::to_string(c) + "," + std::to_string(c) + "," +
                     std::to_string(c) + ",0.8)";
    p.poly(tri, PArg{.fil = co, .str = co, .wid = 0});
  }
}

void Tree::tree08(Painter& p, double x, double y, double hei, double wid,
                  const std::string& col, double noi) {
  double ang = normRand(-1, 1) * PI * 0.2;
  auto tr = branch(BranchArg{.hei = hei, .wid = wid, .ang = -PI / 2 + ang,
                             .det = hei / 20, .ben = PI * 0.2});
  Pts trlist = tr.first;
  trlist.insert(trlist.end(), tr.second.rbegin(), tr.second.rend());
  ...
  sa.out = 0;
  Pts shifted;
  for (const auto& v : trlist) shifted.push_back({v[0] + x, v[1] + y});
  stroke(p, shifted, sa);
  p.absorb(std::move(tw));
}
