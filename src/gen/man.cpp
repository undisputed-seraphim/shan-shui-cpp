...
static std::pair<Pts, Pts> expand(const Pts& ptlist, const WFun& wfun) {
	...
	for (size_t i = 1; i + 1 < ptlist.size(); i++) {
		...
		if (a < a2)
			a += pi;
		vtxlist0.push_back({ptlist[i][0] + w * std::cos(a), ptlist[i][1] + w * std::sin(a)});
		vtxlist1.push_back({ptlist[i][0] - w * std::cos(a), ptlist[i][1] - w * std::sin(a)});
	}
	size_t l = ptlist.size() - 1;
	double a0 = std::atan2(ptlist[1][1] - ptlist[0][1], ptlist[1][0] - ptlist[0][0]) - pi / 2;
	double a1 = std::atan2(ptlist[l][1] - ptlist[l - 1][1], ptlist[l][0] - ptlist[l - 1][0]) - pi / 2;
	double w0 = wfun(0);
	double w1 = wfun(1);
	vtxlist0.insert(vtxlist0.begin(), {ptlist[0][0] + w0 * std::cos(a0), ptlist[0][1] + w0 * std::sin(a0)});
	vtxlist1.insert(vtxlist1.begin(), {ptlist[0][0] - w0 * std::cos(a0), ptlist[0][1] - w0 * std::sin(a0)});
	vtxlist0.push_back({ptlist[l][0] + w1 * std::cos(a1), ptlist[l][1] + w1 * std::sin(a1)});
	vtxlist1.push_back({ptlist[l][0] - w1 * std::cos(a1), ptlist[l][1] - w1 * std::sin(a1)});
...
static Pts tranpoly(const Pt& p0, const Pt& p1, const Pts& ptlist) {
	...
	for (const auto& v : plist) {
		...
		qlist.push_back({p0[0] + d * scl * std::cos(ang + a), p0[1] + d * scl * std::sin(ang + a)});
	}
	return qlist;
}
...
void Man::hat01(Painter& p, const Pt& p0, const Pt& p1, const HatArg& a) {
	...
	p.poly(tranpoly(p0, p1, f(qlist1)), PArg{.str = "rgba(100,100,100,0.8)", .wid = 1});
}
...
void Man::stick01(Painter& p, const Pt& p0, const Pt& p1, const StickArg& a) {
	...
	p.poly(tranpoly(p0, p1, f(qlist1)), PArg{.str = "rgba(100,100,100,0.5)", .wid = 1});
}
...
void Man::man(Painter& p, double xoff, double yoff, const ManArg& a) {
	...
	std::vector<double> len = a.len.empty() ? std::vector<double>{0, 30, 20, 30, 30, 30, 30, 30, 30} : a.len;
	for (auto& v : len)
		v *= sca;
	...
	auto cloth = [&](const Pts& plist, const auto& fun) {
		...
		for (auto& v : tlist1)
			poly1.push_back(toGlobal(v));
		for (auto& v : tlist2)
			poly1.push_back(toGlobal(v));
		p.poly(poly1, PArg{.fil = "white"});
		Pts g1, g2;
		for (auto& v : tlist1)
			g1.push_back(toGlobal(v));
		for (auto& v : tlist2)
			g2.push_back(toGlobal(v));
		stroke(p, g1, SArg{.wid = 1, .col = "rgba(100,100,100,0.5)"});
		stroke(p, g2, SArg{.wid = 1, .col = "rgba(100,100,100,0.6)"});
	};

	auto fsleeve = [&](double x) {
		return sca * 8 * (std::sin(0.5 * x * pi) * std::pow(std::sin(x * pi), 0.1) + (1 - x) * 0.4);
	};
	auto fbody = [&](double x) {
		return sca * 11 * (std::sin(0.5 * x * pi) * std::pow(std::sin(x * pi), 0.1) + (1 - x) * 0.5);
	};
	auto fhead = [&](double x) { return sca * 7 * std::pow(0.25 - std::pow(x - 0.5, 2), 0.3); };

	if (ite)
		ite(p, toGlobal(pts[8]), toGlobal(pts[6]), a.fli);
	...
	Pts hlist = bezier_mid_hull({pts[1], pts[2]}, 2);
	...
	for (auto& v : hlist1)
		hpoly.push_back(toGlobal(v));
	for (auto it = hlist2.rbegin(); it != hlist2.rend(); ++it)
		hpoly.push_back(toGlobal(*it));
