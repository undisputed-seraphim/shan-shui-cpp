...
Pts flip(Pts ptlist, double axis = 0) {
  for (auto& pt : ptlist) pt[0] = axis - (pt[0] - axis);
  return ptlist;
}

std::vector<Pts> flipNested(std::vector<Pts> ptlist, double axis = 0) {
  for (auto& row : ptlist) row = flip(row, axis);
  return ptlist;
}

void hut(Painter& p, double xoff, double yoff, double hei = 40, double wid = 180,
         int tex = 300) {
  ...
  for (int i = 0; i < reso0; i++) {
    ...
    for (int j = 0; j < reso1; j++) {
      double nx = wid * ((double)i / (reso0 - 1) - 0.5) *
                  std::pow((double)j / (reso1 - 1), 0.7);
  ...
  p.poly(ptlist[0], PArg{.xof = xoff, .yof = yoff, .fil = "none",
                         .str = "rgba(100,100,100,0.3)", .wid = 2});
  p.poly(ptlist[ptlist.size() - 1],
         PArg{.xof = xoff, .yof = yoff, .fil = "none",
              .str = "rgba(100,100,100,0.3)", .wid = 2});
  TArg2 ta;
  ta.xof = xoff;
  ta.yof = yoff;
  ...
  ta.len = 0.25;
  ta.col = [](double) {
    return "rgba(120,120,120," + toFixed(0.3 + rnd() * 0.3, 3) + ")";
  };
...
void box(Painter& p, double xoff, double yoff, const BoxArg& a) {
  ...
  Pts polist = {{-a.wid * 0.5, -a.hei}, {a.wid * 0.5, -a.hei}, {a.wid * 0.5, 0},
                {mid, a.per}, {-a.wid * 0.5, 0}};
  if (!a.tra) {
  ...
  for (const auto& row : ptlist) {
    ...
    Pts shifted;
    for (const auto& v : row) shifted.push_back({v[0] + xoff, v[1] + yoff});
    stroke(p, shifted, sa);
  }
}

// deco(style, rect-ish args): returns list of polylines
std::vector<Pts> deco(int style, const std::array<Pt, 4>& r, double hsp0,
                      double hsp1, double vsp0, double vsp1) {
  const Pt& pul = r[0];
  const Pt& pur = r[1];
  const Pt& pdl = r[2];
  const Pt& pdr = r[3];
...
void rail(Painter& p, double xoff, double yoff, double seed, const RailArg& a) {
  ...
  if (a.tra) {
    ptlist.push_back(
        div({{-a.wid * 0.5, -a.hei}, {bmid, -a.hei - a.per}}, a.seg));
    ptlist.push_back(
        div({{bmid, -a.hei - a.per}, {a.wid * 0.5, -a.hei}}, a.seg));
  }
  if (a.tra) {
  ...
  for (size_t i = 0; i < ptlist.size() / 2; i++) {
    for (size_t j = 0; j < ptlist[i].size(); j++) {
      ...
      ptlist[ci][j % ptlist[ci].size()][1] +=
          (nse(i + 0.5, j * 0.5, seed) - 0.5) * a.hei;
      Pts ln = div({ptlist[i][j], ptlist[ci][j % ptlist[ci].size()]}, 2);
      ln[0][0] += (rnd() - 0.5) * a.hei * 0.5;
      p.poly(ln, PArg{.xof = xoff, .yof = yoff, .fil = "none",
                      .str = "rgba(100,100,100,0.5)", .wid = 2});
    }
  }
  for (const auto& row : ptlist) {
    SArg sa;
...
void roof(Painter& p, double xoff, double yoff, const RoofArg& a) {
  auto opf = [&](const Pts& pl) {
    return a.rot < 0.5 ? flip(pl) : pl;
  };
  ...
  std::vector<Pts> ptlist;
  ptlist.push_back(div(opf({{-a.wid * 0.5 + quat, -a.hei - a.per / 2},
                            {-a.wid * 0.5 + quat * 0.5, -a.hei / 2 - a.per / 4},
                            {-a.wid * 0.5 - a.cor, 0}}), 5));
  ptlist.push_back(div(opf({{mid + quat, -a.hei},
                            {(mid + quat + a.wid * 0.5) / 2, -a.hei / 2},
                            {a.wid * 0.5 + a.cor, 0}}), 5));
  ptlist.push_back(div(opf({{mid + quat, -a.hei},
                            {mid + quat / 2, -a.hei / 2 + a.per / 2},
                            {mid + a.cor, a.per}}), 5));
  ...
  Pts polist = opf({{-a.wid * 0.5, 0}, {-a.wid * 0.5 + quat, -a.hei - a.per / 2},
                    {mid + quat, -a.hei}, {a.wid * 0.5, 0}, {mid, a.per}});
  p.poly(polist, PArg{.xof = xoff, .yof = yoff, .fil = "white", .str = "none"});
  ...
  if (a.pla[0] == 1) {
    Pts pp = opf({{mid + quat / 2, -a.hei / 2 + a.per / 2},
                  {-a.wid * 0.5 + quat * 0.5, -a.hei / 2 - a.per / 4}});
    if (pp[0][0] > pp[1][0]) std::swap(pp[0], pp[1]);
...
void Arch::arch01(Painter& p, double xoff, double yoff, double seed,
                  double hei, double wid, double rot, double per) {
...
void Arch::arch02(Painter& p, double xoff, double yoff, double seed,
                  double hei, double wid, double rot, double per, int sto,
                  int sty, bool rai) {
  double hoff = 0;
  for (int i = 0; i < sto; i++) {
    BoxArg ba;
...
void Arch::arch03(Painter& p, double xoff, double yoff, double seed,
                  double hei, double wid, double rot, double per, int sto) {
  double hoff = 0;
  for (int i = 0; i < sto; i++) {
    BoxArg ba;
    ...
    ba.wei = 1.5;
    ba.per = per / 2;
    ba.dec = [](const std::array<Pt, 4>& r) {
      return deco(1, r, 1, 4, 1, 2);
    };
    box(p, xoff, yoff - hoff, ba);
...
void Arch::arch04(Painter& p, double xoff, double yoff, double seed,
                  double hei, double wid, double rot, double per, int sto) {
  double hoff = 0;
  for (int i = 0; i < sto; i++) {
    BoxArg ba;
...
void Arch::boat01(Painter& p, double xoff, double yoff, double seed,
                  double len, double sca, bool fli) {
  double dir = fli ? -1 : 1;
  ManArg ma;
  ma.ite = [](Painter& p2, const Pt& a, const Pt& b, bool f) {
    Man::stick01(p2, a, b, StickArg{.fli = f});
  };
  ma.hat = [](Painter& p2, const Pt& a, const Pt& b, const HatArg& h) {
    Man::hat02(p2, a, b, h);
  };
  ma.sca = 0.5 * sca;
  ma.fli = !fli;
  ...
  sa.col = "rgba(100,100,100,0.4)";
  Pts shifted;
  for (const auto& v : plist) shifted.push_back({xoff + v[0], yoff + v[1]});
  stroke(p, shifted, sa);
}

void Arch::transmissionTower01(Painter& p, double xoff, double yoff, double seed,
                               double hei, double wid) {
  auto toGlobal = [&](const Pt& v) { return Pt{v[0] + xoff, v[1] + yoff}; };
  auto quickstroke = [&](const Pts& pl) {
    Pts shifted;
    for (const auto& v : div(pl, 5)) shifted.push_back(toGlobal(v));
    SArg sa;
    sa.wid = 1;
